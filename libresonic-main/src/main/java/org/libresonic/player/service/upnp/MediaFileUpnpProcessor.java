...
public class MediaFileUpnpProcessor extends UpnpContentProcessor <MediaFile, MediaFile> {
  ...
  public BrowseResult searchByName(String name,
    ...
    List<MediaFile> matches = getMediaFileDao().searchSongsByName(name);

    try {
      List<MediaFile> selectedItems = Util.subList(matches, firstResult, maxResults);
  ...
  public List<MediaFile> getChildren(MediaFile item) {
    ...
    children.sort((MediaFile o1, MediaFile o2)->o1.getName().replaceAll("\\W", "").compareToIgnoreCase(o2.getName().replaceAll("\\W", "")));
    return children;
  }
